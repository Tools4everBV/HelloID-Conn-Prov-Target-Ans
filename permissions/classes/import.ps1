####################################################################
# HelloID-Conn-Prov-Target-Ans-ImportPermissions-Class
# PowerShell V2
####################################################################
...
try {
    ...
    do { 
        ...
        foreach ($class in $FoundClasses) { 
            ...
            $correlatedClass = $requestdetailResult[0]
             if ($null -eq $correlatedClass) {
                Write-Information "Could not retrieve details for class with id $($class.id). Skipping permission entitlement import for this class."
                Continue
             } 
            ...
            # The code below splits a list of permission members into batches of 250
            ...
            $batchSize = 250
             for ($i = 0; $i -lt ($correlatedClass.user_ids | Measure-Object).Count; $i += $batchSize) {
                $permission.AccountReferences = $correlatedClass.user_ids[$i..([Math]::Min($i + $batchSize - 1, $correlatedClass.user_ids.Count - 1))]
    ...
    if ($($ex.Exception.GetType().FullName -eq 'Microsoft.PowerShell.Commands.HttpResponseException') -or
        ...
        Write-Error "Could not import Ans permission entitlements. Error: $($errorObj.FriendlyMessage)"
    }
    else {
        Write-Warning "Error at Line '$($ex.InvocationInfo.ScriptLineNumber)': $($ex.InvocationInfo.Line). Error: $($ex.Exception.Message)"
        Write-Error "Could not import Ans permission entitlements. Error: $($ex.Exception.Message)"
    }
}